...
def insertTweetOwner(records, connection, cursor):
    ...
    sql_insert_query = """ INSERT INTO tweet_owner(id, fetching_time, followers, following, image_url, 
                                    is_most_interacted_user, name, type_of_spreader, username) 
                           VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (id) DO NOTHING"""

    cursor.executemany(sql_insert_query, records)
    connection.commit()
    print(cursor.rowcount, "Records were inserted to tweet_owner table successfully")
    connection.close()
...
def insertTweet(records):
    ...
    cursor.executemany(sql_insert_query, records)
    connection.commit()
    print(cursor.rowcount, "Records were inserted to tweet table successfully")
    connection.close()


def insertTweetForTweetOwners(records):
    ...
    cursor.executemany(sql_insert_query, records)
    connection.commit()
    print(cursor.rowcount, "Records weere inserted to tweet table for owners successfully")
    connection.close()


def updateTweetOwnerForMostInteractedUser(records):

    connection = connectDatabase()
    cursor = connection.cursor()
    print(records)
    sql_insert_query = """ UPDATE tweet_owner SET is_most_interacted_user = %s WHERE  id = %s """

    cursor.executemany(sql_insert_query, records)
    connection.commit()
    print(cursor.rowcount, "Records were updates on tweet_owner table successfully")
    connection.close()
...
def getMostInteractedTweetOwnerIds():
    ...
    sql_select_query = """select owner_id
                          from (
